#!/home/yuziqi/software/miniconda3/envs/RNASEQ/bin/python
'''
...
def genebody_coverage_worker(args):
        '''
        统计一部分转录本的覆盖度
        '''
        bam, items = args
        samfile = pysam.Samfile(bam, "rb")
        aggreagated_cvg = collections.defaultdict(int)
        for chrom, strand, positions in items:
                coverage = {}
                for i in positions:
                        coverage[i] = 0.0
                chrom_start = positions[0]-1
...
def genebody_coverage_parallel(bam, position_list, num_threads=32):
        '''
        多进程统计gene body覆盖度
        '''
        # 按顺序分割转录本
        items = list(position_list.values())
        chunk_size = (len(items) + num_threads - 1) // num_threads
        chunks = [items[i*chunk_size:(i+1)*chunk_size] for i in range(num_threads)]
        args = [(bam, chunk) for chunk in chunks if chunk]
        results = []
        with concurrent.futures.ProcessPoolExecutor(max_workers=num_threads) as executor:
                futures = [executor.submit(genebody_coverage_worker, arg) for arg in args]
...
def main():
        ...
        parser.add_option("-p","--processes",action="store",type="int",dest="num_processes",default=32,help="Number of processes to use for parallel computing. default=%default")
        ...
        for bamfile in bamfiles:
                ...
                cvg = genebody_coverage_parallel(bamfile, gene_percentiles, num_threads=options.num_processes)
                if len(cvg) == 0:
